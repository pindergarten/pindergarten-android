    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        noticeBtn.setOnClickListener {
            ...
            if (user_Id == myId) {
                ...
                val cancelBtn = view?.findViewById<ImageButton>(R.id.cancelBtn)
                val hideBtn = view?.findViewById<ImageButton>(R.id.hideBtn)
                dialog = builder.create()
                dialog!!.setView(view)
                dialog!!.window?.setBackgroundDrawableResource(android.R.color.transparent)
                dialog!!.window!!.setGravity(Gravity.BOTTOM)
                ...
                hideBtn?.setOnClickListener{
                    //숨기기
                    var declare: java.util.HashMap<String, String> = java.util.HashMap()
                    declare["title"] = "숨기기"
                    declare["content"] = "숨기기"
                    apiService.declarePostAPI(sharedPreferences.toString(),postId, declare,1)?.enqueue(object :
                        Callback<Post?> {
                        override fun onResponse(call: Call<Post?>, response: Response<Post?>) {
