...
import com.pindergarten_android.pindergarten_android.databinding.ActivityJoin3Binding
...
class Join3Activity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var notice : ImageView = findViewById(R.id.notice)
        ...
        id?.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(charSequence: CharSequence, i: Int, i1: Int, i2: Int) {
                ...
                else if(id?.text.toString().length<2){
                    info?.text="*2자 이상 입력해주세요."
                    info?.visibility=View.VISIBLE
                    info?.setTextColor(Color.parseColor("#CF0E1E"))
                    confirmIdBtn?.setImageResource(R.drawable.join3_confirm)
                    confirmIdBtn?.isClickable = false
                    notice.visibility = View.VISIBLE
                }
                else{
                    info?.text="*계정 이름에는 영문, 숫자, 밑줄 및 마침표만 사용할 수 있습니다."
                    info?.visibility=View.VISIBLE
                    info?.setTextColor(Color.parseColor("#CF0E1E"))
                    confirmIdBtn?.setImageResource(R.drawable.join3_confirm)
                    confirmIdBtn?.isClickable = false
